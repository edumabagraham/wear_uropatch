...
class NestedCVOptimizer:
                ...
                try:
                    ...
                    for class_name in self.class_names:
                        if f'recall_{class_name}' in fold_metrics:
                            print(f"{model_name} {class_name} recall: {fold_metrics[f'recall_{class_name}']:.4f}")
        ...
        for model_name in self.models:
            if self.outer_scores[model_name]:
                ...
                for class_name in self.class_names:
                    ...
                    if recall_col in scores_df.columns:
                        mean_val = scores_df[recall_col].mean()
                        std_val = scores_df[recall_col].std()
                        print(f"{recall_col:15}: {mean_val:.4f} ± {std_val:.4f}")
    ...
    def get_results_dataframe(self) -> pd.DataFrame:
        ...
        and best hyperparameters. Returns a clean DataFrame ready for analysis.
        """
        results_data = []
