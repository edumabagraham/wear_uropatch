...
class NestedCVOptimizer:
    ...
    def get_model_search_space(self, model_name: str, trial) -> Dict[str, Any]:
        """Define model-specific hyperparameter search space with PCA."""
        # Use variance ratio approach for PCA
        ...
        def objective(trial):
            ...
            for train_idx, val_idx in self.inner_cv.split(X_train_outer, y_train_outer, groups_train_outer):
                try:
                    ...
                    # For binary classification, we need to get probabilities for the positive class
                    if y_proba.shape[1] == 2:
                        y_proba_pos = y_proba[:, self.positive_class_encoded]
                    else:
                        y_proba_pos = y_proba.ravel()
                    
                    # Create binary labels for log loss calculation
                    y_true_binary = (y_val_inner == self.positive_class_encoded).astype(int)
                    
                    # score = -log_loss(y_true_binary, y_proba_pos)
                    score = -log_loss(y_val_inner, y_proba)
                    fold_scores.append(score)
                    
                except Exception as e:
                    print(f"Warning: Error in inner fold for {model_name}: {e}")
                    fold_scores.append(-10.0)  # Very poor score for errors
    ...
    def plot_auc_comparison(self, save_path: str = None, figsize: Tuple[int, int] = (10, 6)):
        ...
        axes[0].set_title('AUC Across Folds')
        ...
        if mean_aucs:
            ...
            axes[1].set_title('Mean AUC Comparison')
