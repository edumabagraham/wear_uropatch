...
class NestedCVOptimizer:
    ...
    def get_model_search_space(self, model_name: str, trial) -> Dict[str, Any]:
        """Define model-specific hyperparameter search space using variance ratio approach for PCA."""
        # Use variance ratio approach for PCA: choose what percentage of variance to retain
        pca_n_components = trial.suggest_categorical('pca__n_components', 
        ...
        if model_name == 'rf':
            params.update({
                ...
                'clf__max_depth': trial.suggest_int('clf__max_depth', None),
    ...
    def create_model(self, model_name: str, params: Dict[str, Any]):
        """Instantiate a Pipeline with PCA and the model with given parameters."""
        # Extract classifier-specific parameters by removing the 'clf__' prefix
        clf_params = {k.replace('clf__', ''): v for k, v in params.items() if k.startswith('clf__')}
        pca_params = {k.replace('pca__', ''): v for k, v in params.items() if k.startswith('pca__')}
        ...
        # Create the PCA instance - using variance ratio approach
        # If n_components is a float between 0-1, PCA will keep enough components to explain that variance ratio
        pca = PCA(**pca_params, random_state=self.random_state)
        ...
        def objective(trial):
            ...
            for train_idx, val_idx in self.inner_cv.split(X_train_outer, y_train_outer, groups_train_outer):
                try:
                    ...
                    # Return a very poor score if error occurs
                    fold_scores.append(-10.0)

            return np.mean(fold_scores) if fold_scores else -10.0

        # Create study for this model - maximize negative log loss
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(self.outer_cv.split(self.X, self.y_encoded, self.groups)):
            ...
            for model_name in self.models:
                ...
                try:
                    ...
                    print(f"Best {model_name} CV score (neg_log_loss): {study.best_value:.4f}")
